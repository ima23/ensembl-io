...
sub init {
  ...
  ## Question about if it is valid to have such an opinion on a BigBed file without knowing the type of data it will
  ## contain. Many other BED formats can have 12 columns without using these names. This only really applies for 
  ## transcript data.
  ...
  if ($autoSQL && $autoSQL->is_table()) {
    ...
    my $default_name_length = scalar @{$self->{'default_names'}};
    foreach my $f (@{$fields}) {
      my $real_name;
      ## Check for incomplete AutoSQL
      if ($f->is_autogenerated() && $i < $default_name_length) { 
